...
function renderFCPlay() {
  ...
  const answerArea = fcChecked
    ...
    : `<input class="answer-input" id="answerInput" type="text" inputmode="text" autocomplete="off" spellcheck="false" value="${fcTyped.replace(/"/g, "&quot;")}">`;
  ...
  root.innerHTML = `
    ...
    <div class="intro">
      ...
      <ol class="steps">
        ...
        <li><span class="step-num">4</span><span class="step-text"><strong>Electrons move by carrying them</strong>: drag a dot from one atom straight onto another to hand it over — that is how an atom with an electron to spare gives it to a neighbor who needs it. For ions, the extra electron comes from some other species entirely: fetch it from the <strong>e⁻ dispenser</strong> next to the table, or drop a dot back on the dispenser to give one up. The Check counts every one.</span></li>
