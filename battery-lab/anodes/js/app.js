/*
  Battery Lab — Anode Materials: app controller.
  Mirrors the cathodes game: study (flashcard gallery) → quiz → done,
  house rules throughout (tabs everywhere, predict-then-Check, misses
  requeue, done screen offers next + revisit + again + home).
  Structures here are schematic mechanism figures (js/fig.js), not molecules.
*/
...
const TIERS = ["intercalation", "alloying", "metal", "all"].map((key) => ({
  key,
...
const TYPE_LABELS = {
  structName: "structure → material",
  nameStruct: "material → structure",
...
/* Unicode sub/superscripts → real markup so CSS controls the size (house
   subscript rule). Applied to facts, claims, taglines, specs. Decimal
   stoichiometry (Li₄.₄Sn) keeps the period inside the one <sub>. */
function chem(s) {
  return esc(s)
    .replace(/[₀-₉]+(?:\.[₀-₉]+)*/g, (m) =>
      `<sub>${[...m].map((ch) => (ch === "." ? "." : SUBS[ch])).join("")}</sub>`
    )
...
function catChip(card) {
  ...
  return `<span class="cat-chip cat-${card.category}">${cat.label}</span>`;
}
...
function studyCard(card) {
  ...
  const head = `
    <button class="mol-head" type="button" data-id="${card.id}" aria-expanded="${open}">
      <div class="mol-figure">${figSvg(card, { aria: card.name })}</div>
      <div class="mol-title">
...
function renderStudy() {
  ...
  root.innerHTML = `
    ...
    <p class="study-hint">Click a card to flip it open. The figures share one style — layered hosts, swelling alloy particles, a bare metal slab — so the storage mechanism reads at a glance.</p>
    ...
    ${cards.length >= 2
      ...
      <span class="quiz-note">${roundLen} questions · structures, formulas, and facts, mixed both ways</span>
    </div>`
      : `<div class="controls quiz-launch">
      <button class="action primary" id="mixBtn" type="button">Drill it in the Full mix →</button>
      <span class="quiz-note">One card can't quiz against itself — the Full mix deals it against the whole deck</span>
    </div>`}`;
  wireTabs();
  ...
  root.querySelector("#mixBtn")?.addEventListener("click", () => {
    tierIndex = TIERS.findIndex((t) => t.key === "all");
    startQuiz();
  });
}
...
function optionInner(opt, q) {
  if (q.type === "nameStruct") {
    ...
    return figSvg(c, { aria: "structure option", className: "quiz-fig" });
  }
...
function questionBlock(q) {
  ...
  if (q.type === "structName") {
    return {
      prompt: "Which anode material is this structure?",
      stage: `<div class="mol-stage">${figSvg(card, { aria: "mystery structure", className: "quiz-fig" })}</div>`,
    };
  }
  if (q.type === "nameStruct") {
    return {
      prompt: `Which structure is <strong>${esc(card.name)}</strong> (${esc(card.abbr)})?`,
      stage: "",
    };
  }
  if (q.type === "claimWho") {
